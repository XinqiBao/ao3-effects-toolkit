...
2. Copy the tap block (`<details class="route-map">…`) from `example.html` into AO3's HTML editor.
...
- This effect works best with three stops. More than that makes the revealed notes crowded in standard AO3 widths.
...
- Tap the route surface to reveal its notes, then tap the same surface to close it.
