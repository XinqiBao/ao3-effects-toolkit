...
2. Copy the tap block (`<details class="casefile">…`) from `example.html` into AO3's HTML editor.
...
- Keep the ticket and note slips short. This effect works best when a tap reveals two small corroborating fragments, not a second full document stack.

## Notes

- AO3 work content does not run JavaScript; this effect is CSS only.
- Tap the dossier surface to reveal the slips, then tap the same surface to close it.
