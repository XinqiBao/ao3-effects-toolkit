...
1. 把 `effects/typewriter/work-skin.css` 全量添加到 AO3 的 Work Skin
2. 把 `effects/typewriter/hover-template.html` 贴进 AO3 的 HTML 编辑器
3. 预览成功后再根据需要改成 tap 版本

你真正需要关心的文件全在 `effects/typewriter/` 目录下：
...
2. 把 `effects/typewriter/work-skin.css` 全量复制进去（可以追加到已有的 work skin 里）
3. 保存
...
1. 正文编辑器切到 **HTML** 模式
2. 复制 `effects/typewriter/hover-template.html` 贴进去
3. 点 **Preview**
...
- 改了 `effects/typewriter/work-skin.css`
- 改了 `effects/typewriter/*.html` 模板文件
- 想确认 AO3 真实解析会不会过滤某种 HTML 或 CSS

### 验证流程

1. 修改 `effects/typewriter/work-skin.css` 或模板文件
2. 在本地打开 `effects/typewriter/preview.html`，检查四种状态
