...
1. 把 `secret-divider/work-skin.css` 全量复制到 AO3 的 Work Skin
2. 把 `smoke-test.html` 贴进 AO3 的 HTML 编辑器（如果有）
3. 预览成功后，再换成正式模板

你真正需要关心的文件全在 `secret-divider/` 目录下：
...
2. 在现有 Skin 末尾追加 `secret-divider/work-skin.css` 的全部内容
3. 保存
...
普通用户只需要关心 `secret-divider/` 下的内容，其他都可以忽略。
...
- 改了 `secret-divider/work-skin.css`
- 改了模板文件
...
3. 如需真实 AO3 验证，脚本会贴入模板并检查 DOM 结构
